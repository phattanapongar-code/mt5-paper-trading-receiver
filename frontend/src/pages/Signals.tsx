...
export default function Signals() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-sm font-semibold text-body">Signal Logs</h1>
          ...
          <tbody>
            {logs.map((log) => (
              <tr key={log.id} className="border-b border-surface-elevated-dark text-text-secondary">
                ...
                <td className="py-2 pr-3 font-mono text-primary">#{log.bot_id}</td>
            ...
            {logs.length === 0 && (
              <tr><td colSpan={4} className="py-4 text-center text-muted">No signal logs</td></tr>
            )}
          </tbody>
        </table>
      </div>
