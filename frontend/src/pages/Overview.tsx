  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          <span className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full ${health?.sender_online ? 'bg-cyber-green shadow-[0_0_6px_#00ff88]' : 'bg-cyber-red shadow-[0_0_6px_#ff3355]'}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card label="Bid" value={tick?.bid?.toFixed(2) ?? '—'} accent="cyan" />
        <Card label="Ask" value={tick?.ask?.toFixed(2) ?? '—'} accent="cyan" />
        <Card label="Spread" value={tick?.spread?.toFixed(1) ?? '—'} accent="yellow" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card
          ...
          accent={paper?.realized_pnl != null && paper.realized_pnl >= 0 ? 'green' : 'red'}
        />
        <Card
          label="Auto Execution"
          value={exec?.enabled ? 'ON' : 'OFF'}
          accent={exec?.enabled ? 'green' : 'red'}
        />
      </div>
      ...
      {paper?.open_position && (
        <div className="bg-surface-800 border border-surface-500 rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-text-muted text-xs">Side</span>
              <p className={`font-mono font-semibold ${paper.open_position.side === 'buy' ? 'text-cyber-green' : 'text-cyber-red'}`}>
                {paper.open_position.side.toUpperCase()}
              </p>
            </div>
            <div>
              <span className="text-text-muted text-xs">Lot</span>
            ...
            <div>
              <span className="text-text-muted text-xs">SL / TP</span>
      ...
      {trades.length > 0 && (
        <div>
          <h2 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Recent Trades</h2>
...
function Card({ label, value, accent }: { label: string; value: string; accent: 'cyan' | 'green' | 'red' | 'yellow' | 'muted' }) {
  const accentMap = {
    ...
    muted: 'border-surface-400 text-text-secondary',
  }
  return (
    <div className="bg-surface-800 border border-surface-500 rounded-lg p-4">
