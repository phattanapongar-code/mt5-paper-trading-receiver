...
export default function BotManager() {
  ...
  const [visualStrategies, setVisualStrategies] = useState<VisualStrategyItem[]>([])
  const navigate = useNavigate()

  const fetchData = useCallback(async () => {
    try {
      const [profRes, botsRes, vsRes] = await Promise.all([
        client.get<Profile[]>('/profiles'),
        client.get<Bot[]>('/bots'),
        client.get<VisualStrategyItem[]>('/visual-strategies'),
      ])
  ...
  const saveEdit = useCallback(async () => {
    ...
    try {
      await client.put(`/bots/${editBot.bot.id}`, {
        ...
        visual_strategy_id: editBot.visual_strategy_id,
      })
      setEditBot(null)
          ...
          <div key={profile.id} className="bg-surface-card-dark border border-hairline-on-dark rounded-lg overflow-hidden">
            ...
            {profileBots.length > 0 ? (
              <div className="divide-y divide-surface-elevated-dark">
                {profileBots.map((bot) => {
                  const vs = bot.visual_strategy_id
                    ? visualStrategies.find((v) => v.id === bot.visual_strategy_id)
                    : null
                  return (
                    <div key={bot.id}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-mono text-muted">{vs ? vs.name : 'Visual'}</span>
                        <span className="text-xs font-mono text-muted">{bot.symbol} {bot.timeframe}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={(e) => { e.stopPropagation(); setEditBot({ bot, name: bot.name, symbol: bot.symbol, timeframe: bot.timeframe, visual_strategy_id: bot.visual_strategy_id ?? null }) }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setEditBot(null)}>
          <div className="bg-surface-card-dark border border-hairline-on-dark rounded-lg p-5 w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs text-muted mb-1">Visual Strategy</label>
                <select value={editBot.visual_strategy_id ?? ''} onChange={(e) => setEditBot({ ...editBot, visual_strategy_id: e.target.value ? Number(e.target.value) : null })}
                  className="w-full px-3 py-2 bg-surface-elevated-dark border border-hairline-on-dark rounded text-sm text-body focus:outline-none focus:border-primary">
