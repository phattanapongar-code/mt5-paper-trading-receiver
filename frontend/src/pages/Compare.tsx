...
export default function Compare() {
  ...
  if (!data.length) return <div className="text-text-muted text-xs p-6">No bots to compare.</div>

  return (
    <div className="p-6 space-y-4">
      <h1 className="text-sm font-semibold text-text-primary">Compare Bots</h1>
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {data.map((b) => (
              <tr key={b.bot_id} className="border-b border-surface-600 text-text-secondary">
                <td className="py-2 pr-3">{b.name}
                  <span className="ml-2 text-text-muted">{b.profile_name}</span>
                </td>
                ...
                <td className={`text-right py-2 pr-3 font-mono ${b.net_pnl >= 0 ? 'text-cyber-green' : 'text-rose-500'}`}>
