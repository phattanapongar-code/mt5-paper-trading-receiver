  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-text-primary">Pending Orders</h1>
        <button
          onClick={evaluateAll}
          className="px-3 py-1.5 text-xs rounded-md bg-cyber-cyan/20 text-cyber-cyan border border-cyber-cyan/50 hover:bg-cyber-cyan/30 transition-colors cursor-pointer"
        >
          Re-evaluate
        </button>
      </div>
      ...
      {state?.rules && (
        <div className="bg-surface-800 border border-surface-500 rounded-lg p-4">
          <p className="text-xs text-text-muted mb-2 uppercase tracking-wider font-semibold">Rules</p>
      ...
      {activeOrder && (
        <div className="bg-surface-800 border border-cyber-cyan/30 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold text-cyber-cyan uppercase tracking-wider">Active Pending Order</p>
            <button
              onClick={() => cancelOrder(activeOrder.id)}
              className="px-2.5 py-1 text-xs rounded bg-cyber-red/20 text-cyber-red border border-cyber-red/50 hover:bg-cyber-red/30 transition-colors cursor-pointer"
            >
              Cancel
            </button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div>
              ...
              <p className={`font-mono text-sm font-semibold ${activeOrder.side === 'buy' ? 'text-cyber-green' : 'text-cyber-red'}`}>
            ...
            <div>
              <p className="text-xs text-text-muted">SL</p>
              <p className="font-mono text-sm">{activeOrder.stop_loss.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-xs text-text-muted">TP</p>
              <p className="font-mono text-sm">{activeOrder.take_profit.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-xs text-text-muted">RR</p>
              <p className="font-mono text-sm text-cyber-yellow">{activeOrder.risk_reward.toFixed(2)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!activeOrder && (
        <div className="bg-surface-800 border border-surface-500 rounded-lg p-8 text-center">
          <p className="text-sm text-text-muted">No active pending orders</p>
        </div>
      )}

      {orders.length > 0 && (
        <div>
          <h2 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">All Pending Orders</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {orders.filter((o) => o.status !== 'pending').map((o) => (
                  <tr key={o.id} className="border-b border-surface-600">
                    <td className={`p-3 font-mono text-xs font-semibold ${o.side === 'buy' ? 'text-cyber-green' : 'text-cyber-red'}`}>
                      {o.side.toUpperCase()}
                    </td>
                    ...
                    <td className="p-3">
                      <span className="text-xs font-mono text-text-muted">{o.status}</span>
                    </td>
                    <td className="p-3">
                      {o.status === 'pending' && (
                        <button
                          onClick={() => cancelOrder(o.id)}
                          className="px-2 py-1 text-xs rounded bg-cyber-red/20 text-cyber-red border border-cyber-red/50 cursor-pointer"
                        >
                          Cancel
                        </button>
                      )}
      ...
      {rejections.length > 0 && (
        <div>
          <h2 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Rejection Log</h2>
          <div className="bg-surface-800 border border-surface-500 rounded-lg overflow-hidden max-h-60 overflow-y-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-500 text-text-muted text-xs sticky top-0 bg-surface-800">
              ...
              <tbody>
                {rejections.map((r: any) => (
                  <tr key={r.id} className="border-b border-surface-600">
                    <td className="p-3 text-xs text-text-muted font-mono">
                      {new Date((r.created_at ?? r.ts) * 1000).toLocaleTimeString()}
                    </td>
                    <td className="p-3 text-xs text-text-muted">{r.reason ?? r.message ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
