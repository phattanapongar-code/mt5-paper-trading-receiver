...
export default function OBInRangeNode({ data }: { data: { params: Record<string, unknown>; label: string; color?: string } }) {
  const { label, color = '#a1887f' } = data
  return (
    <div className="relative w-full">
      <div className="h-full px-3 py-2 bg-surface-card-dark border border-hairline-on-dark rounded-md shadow-lg" style={{ borderColor: color }}>
        ...
        <p className="text-[10px] text-muted mt-1">Price in OB zone</p>
      </div>
