...
export default function BotForm({ profiles, onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    client.get<VisualStrategyItem[]>('/visual-strategies').then((res) => {
      setVisualStrategies(res.data)
    }).catch(() => {})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await client.post('/bots', {
        ...
        symbol,
        timeframe,
        visual_strategy_id: visualStrategyId,
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <select value={visualStrategyId ?? ''} onChange={(e) => setVisualStrategyId(e.target.value ? Number(e.target.value) : null)}
              className="w-full px-3 py-2 bg-surface-elevated-dark border border-hairline-on-dark rounded text-sm text-body focus:outline-none focus:border-primary">
              <option value="">-- No visual strategy --</option>
              {visualStrategies.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
            {visualStrategies.find((s) => s.id === visualStrategyId) && (
              <p className="text-xs text-muted mt-1">{visualStrategies.find((s) => s.id === visualStrategyId)!.description}</p>
            )}
          </div>
