...
app = FastAPI(title="MT5 Paper Trading Receiver", version="1.0.0-final")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def receive_price(payload: TickPayload) -> dict[str, Any]:
    ...
    # Never stage a second setup while a paper position is open. Existing
    # pending candidates are cancelled to keep exposure deterministic.
    if paper.open_trade() is not None:
        active_pending = pending_orders.active(payload.symbol)
        cancelled_pending = pending_orders.cancel(int(active_pending["id"]), "position_already_open") if active_pending else None
        pending_order_refresh = {"created": None, "cancelled": cancelled_pending, "rejected": None}
    else:
        pending_order_refresh = pending_orders.on_tick(payload.symbol, payload.bid, payload.ask, now)
    execution_refresh = execution.on_tick(payload.symbol, payload.bid, payload.ask)
    ...
    event = {
        ...
        "pending_order_refresh": pending_order_refresh,
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "last_seq": last_seq,
        "strategy_enabled": execution.enabled,
...
def state() -> dict[str, Any]:
    ...
    return {
        ...
        "pending_orders": pending_orders.state(settings.symbol),
...
def get_pending_order_state() -> dict[str, Any]:
    return pending_orders.state(settings.symbol)


@app.get("/api/pending-orders")
def get_pending_orders(limit: int = 50) -> list[dict[str, Any]]:
    return pending_orders.list(settings.symbol, max(1, min(limit, 1000)))


@app.get("/api/pending-orders/rejections")
def get_pending_rejections(limit: int = 50) -> list[dict[str, Any]]:
    return pending_orders.rejections(settings.symbol, max(1, min(limit, 1000)))
...
def evaluate_pending_orders() -> dict[str, Any]:
    ...
    return pending_orders.on_tick(
        latest_tick["symbol"], latest_tick["bid"], latest_tick["ask"], latest_tick["received_at"]
    )
...
def cancel_pending_order(order_id: int) -> dict[str, Any]:
    try:
        return pending_orders.cancel(order_id, "manual_cancel")
...
def strategy_enable() -> dict[str, Any]:
    return execution.set_enabled(True)


@app.post("/api/strategy/disable")
def strategy_disable() -> dict[str, Any]:
    return execution.set_enabled(False)
...
from app.multibot.middleware import MultiBotTickMiddleware
app.add_middleware(MultiBotTickMiddleware)
